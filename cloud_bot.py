...
def send_discord_alert(embeds):
    ...
    try:
        requests.post(DISCORD_WEBHOOK_URL, json={"username": "뉴스 요약 봇", "embeds": embeds}, timeout=10)
    except: pass
...
def check_critical_patterns(title):
    sports_keywords = ["프로농구", "KBL", "프로야구", "KBO", "프로축구", "K리그", "선수", "득점", "리바운드", "홈런",
                       "페가수스", "라이온즈", "대구FC", "실내체육관", "끝내기", "스포츠", "MVP", "매치"]
    politics_keywords = ["국회의원", "시의원", "도의원", "구의원", "시장", "군수", "구청장", "도지사", "정치", "후보",
                         "공천", "당선", "선거", "여당", "야당", "국회", "민주당", "더불어민주당", "국민의힘", "국힘",
                         "우세", "추격", "경선", "여론조사", "지지율", "출마", "득표", "총선", "지선", "대선", "최고위", "원내대표"]
    stock_keywords = ["주가", "상승", "하락", "급등", "급락", "증시", "코스피", "코스닥", "종목", "시황",
                      "주식", "매수", "매도", "개미", "외인", "기관", "상장", "공모"]

    issue_crime = ["횡령", "배임", "비리", "탈세", "구속", "압수수색", "기소", "입건", "송치", "체포",
                   "비자금", "가공거래", "허위세금계산서", "페이퍼컴퍼니", "의혹", "혐의", "탈루", "밀약"]
    issue_finance = ["가업승계", "편법증여", "일감몰아주기", "일감 몰아주기", "지분매각", "전환사채", "CB",
                     "신주인수권부사채", "BW", "비상장주식", "우회상장", "자본잠식"]
    ...
    # [수정 2] 핵심 리스크 키워드 먼저 체크 → 스포츠/정치 필터보다 우선
    has_crime_risk = any(word in title for word in issue_crime)
    has_finance_risk = any(word in title for word in issue_finance)

    # 스포츠/정치/주식 필터: 핵심 리스크가 없을 때만 차단
    if not has_crime_risk and not has_finance_risk:
        if any(sport in title for sport in sports_keywords): return -1, "", False
        if any(pol in title for pol in politics_keywords): return -1, "", False
        if any(stock in title for stock in stock_keywords): return -1, "", False
...
def search_naver_news(keyword):
    ...
    try:
        return requests.get(url, headers=headers, params=params, timeout=5).json().get('items', [])
    except: return []
...
def search_google_news(keyword, lang='ko'):
    ...
    try:
        ...
        for item in soup.find_all('item')[:10]:
            # [수정 3] guid로 링크 추출 (link 태그 파싱 불안정 문제 해결)
            link = item.guid.text if item.guid else (item.find('link').next_sibling or '')
            items.append({'title': item.title.text, 'link': link, 'pubDate': item.pubDate.text})
        return items
    except: return []
    ...
    try:
        res = requests.post(
            ...
            json={
                "model": model_name,
                "messages": [
                    {"role": "system", "content": "You are a data deduping AI. Output JSON array only."},
        ...
        if res.status_code == 200:
            ...
            if marker in raw_text:
                raw_text = re.sub(f'{marker}(json)?|{marker}', '', raw_text)
            parsed = json.loads(raw_text.strip())
            deduped = [logs[item['index']] for item in parsed if 'index' in item and 0 <= item['index'] < len(logs)]
            if deduped:
                return deduped
    except Exception as e:
        print(f"⚠️ 데스킹 AI 오류 (원본 유지): {e}")

    # 실패 시 원본 반환
    return logs

# =========================================================
# [그록 429] Groq API 호출 with 재시도 로직
# =========================================================
def call_groq_with_retry(model_name, messages, temperature=0.1, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if res.status_code == 200:
                ...
                wait_sec = 2 ** attempt * 3  # 3초, 6초, 12초
                print(f"⚠️ Groq 429 Rate Limit → {wait_sec}초 대기 후 재시도 ({attempt+1}/{max_retries})")
                time.sleep(wait_sec)
            else:
                print(f"⚠️ Groq 오류 {res.status_code}: {res.text[:100]}")
                break
        except Exception as e:
...
def main():
    ...
    history_url_set = set(history["urls"])  # [수정 5] O(1) 조회를 위한 set 변환
    execution_logs = []
    raw_articles = []
    unique_links = set()
    ...
    if TEST_MODE:
        ...
        lookback = timedelta(hours=24)
        print("\n🌅 [모닝 브리핑 모드] 전날 밤~오늘 아침까지의 주요 기사를 수집합니다.")
    else:
    ...
    for art in raw_articles:
        ...
        try:
            ...
            # [수정 5] history 체크를 valid_articles 구성 시점에 수행 (set으로 O(1) 조회)
            if link in history_url_set:
                continue
        ...
        if score >= 50:
            valid_articles.append({
                ...
                'rule_score': score,  # [수정 6] 룰 기반 점수 별도 보존
                'reason': reason, 'track': track, 'need_ai': need_ai, 'raw': art
            })
...
[세무/재무], [자본이동], [사고/재난], [경영/갈득], [사법/인사], [일반동향]
포맷: {{ "score": 85, "category": "분류", "reason": "[세무/재무]" }}"""
            ...
            # [그록-429] 재시도 로직 적용
            result = call_groq_with_retry(active_model, messages, temperature=0.1)
            ...
            if result:
                try:
                    ...
                    ai_score = parsed.get('score', v['score'])
                    ai_reason = parsed.get('reason', v['reason'])

                    # [수정 6] 룰 기반 점수를 floor로 보장 (AI가 점수를 내리지 못함)
                    v['score'] = max(v['rule_score'], ai_score)

                    # [수정 7] 지역 언론 트랙에서 AI가 0점 판정 시 명시적 폐기
                    if v['track'] == 'local' and ai_score == 0:
                        print(f"   ↳ AI 폐기 판정: {v['title'][:30]}")
                        v['score'] = 0  # floor 무시하고 강제 0 처리
                        v['reason'] = "[AI 폐기]"
                    else:
                        v['reason'] = ai_reason

                except (json.JSONDecodeError, KeyError) as e:
                    # [그록-JSON] 파싱 실패 시 fallback: 룰 기반 점수/태그 유지
                    print(f"   ↳ JSON 파싱 실패 (룰 기반 유지): {e}")
        ...
        if v['score'] >= 50:
            ...
            # [수정 5] history에 추가 + set에도 동시 추가
            history["urls"].append(v['link'])
            history["titles"].append(v['title'])
            history_url_set.add(v['link'])

    # [수정 1] 바깥 폴백 제거: deduplicate 함수 내부에서 실패 시 원본 반환
    ...
    if not final_logs:
        ...
        if high:
            desc += "🚨 **[핵심 리스크 / 징후]**\n"
            for l in high:
                desc += f"**[{l['score']}점]** {l['reason']} [{l['title']}]({l['link']})\n\n"
        if med:
            desc += "🏢 **[대규모 자본이동 및 동향]**\n"
            for l in med:
                desc += f"**[{l['score']}점]** {l['reason']} [{l['title']}]({l['link']})\n\n"
